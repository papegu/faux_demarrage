...
# --- Plot confusion matrix and calibration --- 
...
if 'faux_demarage' not in df.columns:
    raise ValueError("Missing 'faux_demarage' column in dataset.")
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_lstm, y_lstm), 1):
    ...
    # --- Model architecture: LSTM + GRU + Transformer --- 
    ...
    dense_proj = tf.keras.layers.Dense(32)(input_layer)
    attn_out = tf.keras.layers.MultiHeadAttention(num_heads=8, key_dim=16)(dense_proj, dense_proj)
...
print("Average metrics:", np.mean(metrics_per_fold, axis=0))
print("Standard deviation:", np.std(metrics_per_fold, axis=0))
...
# --- Save Metrics to CSV --- 
metrics_df = pd.DataFrame(metrics_per_fold, columns=['Accuracy', 'AUC', 'Precision', 'Recall', 'F1'])
metrics_df.to_csv('metrics_lstm_gru_transformer.csv', index=False)  
# Save the model
